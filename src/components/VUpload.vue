...
import { IMG_URL } from '.././store/imgURL'
export default {
  name: 'VUpload',
  props: {
  ...
  methods: {
    ...
    upload () {
      ...
      let setting = {
        ...
        downtoken_url: 'http://stg-api.meikeapp.party/api/v1/images/downtoken',
        ...
        init: {
          ...
          FileUploaded (up, file, info) {
            ...
            let sourceLink = res.url // 获取上传成功后的文件的Url
            
            that.IMG_URL({
              key:res.key,
              url:res.url,
              imgKeys:that.imgKeys
            })
          ...
          UploadComplete () {
            ...
            for(var i =0 ;i<this.files.length;i++) {
              if(flag) {
                var imgstr = ''
                that.imgKeys=imgstr+this.files[i].target_name
                flag = false;
              }
              else {
                that.imgKeys=that.imgKeys+","+this.files[i].target_name
              }
            }
            that.$emit('getfiles', this.files)
            // that.$emit('imgKey',this.files.key)
            // reset components
            that.uploadInit()
          },
          Key (up, file) {
